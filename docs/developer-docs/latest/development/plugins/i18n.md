...
```
<!-- ? or is it npm install strapi-plugin-i18n? -->
<!-- ? if `npm run strapi install i18n` is not appropriate, should we also update GraphQL plugin install instructions (GraphQL doc read `npm run strapi install graphql`)? -->
...
- The `locale` & `localizations` fields are added to the [GraphQL](https://graphql.org/) schema.
- The GraphQL [query API](/developer-docs/latest/development/plugins/graphql.md#query-api) can be used to:
  - [query with the `locale` argument](#getting-localized-entries-with-graphql) on Collection Types & Single Types
  - [create new localizations with a new mutation](#creating-new-localized-entries-with-graphql) for Collection Types & Single Types
  - filter on [update](#updating-a-localized-entry-with-graphql) & [delete](#deleting-a-localized-entry-with-graphql) of the Single Types
...
### Updating a localized entry with GraphQL
...
### Deleting a localized entry with GraphQL
...
```graphql
{
  createRestaurantLocalization(
    input: { where: { id: 1 }, data: { locale: "xxx", name: "MyRestaurant" } }
  ) {
    restaurant {
      id
      name
...
```graphql
{
  createHomepageLocalization(
    input: { data: { locale: "xxx", name: "MyRestaurant" } }
  ) {
    restaurant {
      id
      name
...
```graphql
{
  updateHomepage(locale: "xxx", input: { data: {...}}) {
    restaurant {
      id
      name
    }
